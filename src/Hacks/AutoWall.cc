...
bool TraceToExit(CVector &end, CTrace &enterTrace, CVector start,
  ...
  while (distance <= 90.0f) {
    ...
    int pointContents
      = interfaces::engineTrace->GetPointContents(end, MASK_SHOT_HULL | CONTENTS_HITBOX, nullptr);
...
bool HandleBulletPenetration(CCSWeaponInfo* weaponInfo, FireBulletData &data) {
  ...
  float enterSurfPenetrationMod = enterSurfaceData->game.penetrationModifier;
  ...
  float exitSurfPenetrationMod = exitSurfaceData->game.penetrationModifier;
  ...
  float v34 = std::fmaxf(0.f, 1.0f / combinedPenetrationModifier);
...
float GetDamage(const CVector &dest, bool teamCheck, FireBulletData &fData) {
  ...
  FireBulletData data = {
    ...
    .filter = CTraceFilter{localPlayer},
  };

  data.direction = (dest - data.src).ToAngle().ToDirection();
